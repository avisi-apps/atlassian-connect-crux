...
(defn update-host! [{:keys [prev-host
                            new-host
                            crux-node]}]
  (loop [tries 0
         host prev-host]
    (when (= tries 4)
      (throw (ex-info "Failed handling lifecycle payload" {:amount-of-times-tried tries})))
    (let [id (:crux.db/id new-host)
      ...
      (when-not (crux/tx-committed? crux-node tx)
        (recur (inc tries) (crux/entity (crux/db crux-node) id))))))
...
(defn handle-lifecycle-payload! [crux-node request]
  (let [lifecycle-payload (namespace-all-keys (:body-params request))
        _ (log/info (select-keys lifecycle-payload [:atlassian-connect.host/base-url
                                                    ...
                                                    :atlassian-connect.host/event-type]) "Handling host lifecycle call")
    ...
    (update-host! {:prev-host prev-lifecycle
