...
(defn jira-admin? [env]
  (true? (get-in env [:claims :context :admin?])))
...
(def atlassian-connect-plugin
  {::p/wrap-read (fn [reader]
                   (fn [env]
                     (if (and (::only-for-jira-admin? (get-resolver-params env))
                              (not (jira-admin? env)))
                       (throw (ex-info "No permissions" {:error :no-access
                                                         :message "Jira admin rights needed"}))
                       (reader env))))
   ::p/wrap-mutate
   (fn [mutate]
     (fn [env k params]
       (if (and (get-in env [::pc/indexes ::pc/index-mutations k ::only-for-jira-admin?])
                (not (jira-admin? env)))
         (throw (ex-info "No permissions" {:error :no-access
                                           :message "Jira admin rights needed"}))
         (mutate env k params))))})
