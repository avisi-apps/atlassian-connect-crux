...
(def test-opts {:crux.node/topology '[crux.standalone/topology crux.kv.rocksdb/kv-store]
                :crux.kv/sync? false})

(defn with-crux [f]
  (let [event-log-dir (str (cio/create-tmpdir "event-log-dir"))]
    (try
      (with-open [crux-node (crux/start-node (assoc
                                               test-opts
...
(t/deftest should-retry-save-host-on-failed-cas-op
  (t/is (nil? (connect-crux/update-host! {:prev-host nil
                                          :new-host {:atlassian-connect.host/client-key "test"
                                                     :atlassian-connect.host/event-type "installed"
                                                     :crux.db/id (connect-crux/client-key->crux-id "test")}
                                          :crux-node *crux-node*})))
  (t/is (nil? (connect-crux/update-host! {:prev-host {:atlassian-connect.host/client-key "test"
                                          ...
                                          :new-host {:atlassian-connect.host/client-key "test"
                                                     ...
                                                     :crux.db/id (connect-crux/client-key->crux-id "test")}
                                          :crux-node *crux-node*})))
  (t/is (= {:atlassian-connect.host/client-key "test"
            :atlassian-connect.host/event-type "enabled"
            :crux.db/id (connect-crux/client-key->crux-id "test")}
